...
export function parseSpecialized(xmlContent: string): SpecializedParseResult {
  ...
  if (!doc) {
    throw new ParseError('No <Document> root in specialized XML', 'no_recognizable_header');
  }

  const entete = doc.Entete;
  if (!entete) {
    throw new ParseError('No <Entete> in specialized XML', 'no_recognizable_header');
  }
