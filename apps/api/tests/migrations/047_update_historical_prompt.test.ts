...
describeIfDb('migration 047 — update_historical_prompt', () => {
  let ctx: MigrationsTestContext;
  let tenantId: string;
  let authorId: string;

  beforeAll(async () => {
    // Stop at 042: 043 + 047 are applied manually below with the seed
    // session vars set, so they actually mutate prompt_bank (the
    // in-loop application would skip both via missing-ok guards).
    // 044/045/046 are intentionally NOT applied here — this test
    // isolates the 047 update against the post-043 state, so the
    ...
    const t = await ctx.testPool.query<{ id: string }>(
      `INSERT INTO tenants (slug, legal_name)
         VALUES ('tenant-test-047', 'Legal Test 047') RETURNING id`,
    );
    tenantId = t.rows[0]!.id;

    const u = await ctx.testPool.query<{ id: string }>(
      `INSERT INTO users (tenant_id, external_sso_id, email, full_name)
         VALUES ($1, 'sso-047-author', 'author-047@tenant-001.example', 'Author 047')
         RETURNING id`,
      [tenantId],
    );
    ...
    const update047 = await readFile(
      join(MIGRATIONS_DIR, '047_update_historical_prompt.sql'),
      'utf8',
    );
  ...
  it('replaces the placeholder template with the real prompt', async () => {
    ...
    expect(template).not.toContain('[HISTORICAL_COMPARE_RUNS_HISTORY_V1]');
    expect(template).toContain('{current_run}');
    expect(template).toContain('{previous_runs}');
    expect(template).toContain('tendance');
    expect(template).toContain('projection_prochain_arrete');
  });
  ...
  it('declares the structured JSON output keys expected by the agent', async () => {
    ...
    const outputKeys = [
      ...
      'annexes_en_deterioration',
      'annexes_en_amelioration',
      'commentaire',
      'projection_prochain_arrete',
      'runs_compares',
    ];
  ...
  it('contains the empty-history fallback instruction', async () => {
    const { rows } = await ctx.testPool.query<{ template: string }>(
      `SELECT template FROM prompt_bank
         WHERE tenant_id = $1
           AND agent_type = 'historical'
    ...
    const template = rows[0]!.template;
    expect(template).toContain('Aucun run historique');
    expect(template).toContain('indetermine');
  });

  it('keeps the runtime parameters unchanged (temperature 0.5, no thinking)', async () => {
    ...
    expect(rows).toHaveLength(1);
    expect(Number(rows[0]!.temperature)).toBe(0.5);
    expect(rows[0]!.thinking_enabled).toBe(false);
    expect(rows[0]!.max_tokens).toBe(4096);
  ...
  it('does not modify any other prompt_bank row', async () => {
    ...
    // 22 seeded - 1 historical updated = 21 remaining placeholders.
