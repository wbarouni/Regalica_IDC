...
describeIfDb('migration 045 — update_investigator_prompt', () => {
  let ctx: MigrationsTestContext;
  let tenantId: string;
  let authorId: string;

  beforeAll(async () => {
    // Stop at 042: 043 + 045 are applied manually below with the seed
    // session vars set, so they actually mutate prompt_bank (the
    // in-loop application would skip both via missing-ok guards).
    // 044 is intentionally NOT applied here — this test isolates the
    // 045 update against the post-043 state, so the count of remaining
    // placeholders is deterministic and independent of 044.
    ctx = await setupMigrationsSchema(42);

    const t = await ctx.testPool.query<{ id: string }>(
      `INSERT INTO tenants (slug, legal_name)
         VALUES ('tenant-test-045', 'Legal Test 045') RETURNING id`,
    );
    tenantId = t.rows[0]!.id;

    const u = await ctx.testPool.query<{ id: string }>(
      `INSERT INTO users (tenant_id, external_sso_id, email, full_name)
         VALUES ($1, 'sso-045-author', 'author-045@tenant-001.example', 'Author 045')
         RETURNING id`,
      [tenantId],
    );
    ...
    const update045 = await readFile(
      join(MIGRATIONS_DIR, '045_update_investigator_prompt.sql'),
      'utf8',
    );
  ...
  it('replaces the placeholder template with the real prompt', async () => {
    ...
    expect(template).not.toContain('[INVESTIGATOR_ANALYZE_FAIL_V1]');
    expect(template).toContain('{fail_data}');
  ...
  it('does not modify any other prompt_bank row', async () => {
    ...
    // 22 seeded - 1 investigator updated = 21 remaining placeholders.
