/**
 * Phases A–E helpers for the BCT evaluation engine.
 * Kept separate to respect the 300-line SonarQube limit.
 */
import Decimal from 'decimal.js';

import type { RuleTerm } from '../../db/models/rule-term';
import { parseBctXml, type CellMatrix } from '../ingestor/bct-xml-parser';
import type { EvaluationVerdict, RuleWithTerms, VerdictStatus } from './types';

Decimal.set({ precision: 38, rounding: Decimal.ROUND_HALF_EVEN });

// ---------------------------------------------------------------------------
// Phase A — Parse XML files into a 3-level dict keyed by annexeCode
// ---------------------------------------------------------------------------
// CellMatrix already has annexeCode as the first key: Map<annexe, Map<rub, Map<col, Decimal>>>
...
export function phaseA(xmlFiles: Map<string, string>): ParsedXmlMap {
  ...
  for (const [, content] of xmlFiles) {
    ...
    // Merge per-file cell matrices into the global map
...
// ---------------------------------------------------------------------------
// Phase B — Group rules by composite key (annexe_code, num_regle)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Phase C — Resolve terms for a single rule
// ---------------------------------------------------------------------------
...
function normalizeAx(raw: string): string {
  // Annexe alias: "0"="00", "1"="01" — strip leading zeros
  return raw.replace(/^0+/, '') || '0';
}

function normalizeCol(raw: string): string {
  return raw.replace(/^0+/, '') || '0';
}
...
export function resolveTerms(
  ...
  return terms.map((t): ResolvedTerm => {
    if (t.kind === 'literal_text') {
      return { skip: 'SKIPPED_LITERAL_TEXT' };
    }
    if (t.kind === 'literal') {
      return { value: new Decimal(String(t.literal_value ?? '0')), term: t };
    }
    // cell_ref
    ...
    const colNorm = normalizeCol(t.colonne ?? '');
    const val = colMap.get(colNorm);
...
// ---------------------------------------------------------------------------
// Phase D — Aggregate by rang (1=LHS, 2=RHS, 3=modifier)
// ---------------------------------------------------------------------------
export type AggResult = { lhs: Decimal; rhs: Decimal } | { skip: 'SKIPPED_DIVISION_BY_ZERO' };
...
function foldTerms(terms: Array<{ value: Decimal; term: RuleTerm }>): Decimal | null {
  ...
  for (const { value: v, term: t } of terms) {
    switch (t.term_op) {
      ...
      case '/':
        if (v.isZero()) return null; // signals division by zero
        acc = acc.dividedBy(v);
        break;
    }
  }
...
export function phaseD(
...
): { lhs: Decimal; rhs: Decimal } | { skip: VerdictStatus } {
  // Check for any skip signals from Phase C
  for (const r of resolved) {
    if ('skip' in r) return { skip: r.skip };
  }

  const validTerms = resolved as Array<{ value: Decimal; term: RuleTerm }>;

  const rang1 = validTerms.filter((t) => t.term.rang === 1);
  const rang2 = validTerms.filter((t) => t.term.rang === 2);
  ...
  if (lhsResult === null || rhsResult === null) {
    return { skip: 'SKIPPED_MISSING_COLONNE' }; // division by zero mapped as skip
  }
...
export function phaseE(
  rule: RuleWithTerms,
  aggResult: { lhs: Decimal; rhs: Decimal } | { skip: VerdictStatus },
): EvaluationVerdict {
  const base = {
    ruleId: rule.id,
    annexeCode: rule.annexe_code,
    numRegle: rule.num_regle,
    operRegle: rule.oper_regle,
  };
  ...
  const op = rule.oper_regle;

  let pass: boolean;
  switch (op) {
    case '=':
    case 'SUM': pass = lhs.equals(rhs); break;
    case '>=':  pass = lhs.gte(rhs); break;
    case '<=':  pass = lhs.lte(rhs); break;
    case '>':   pass = lhs.gt(rhs); break;
    case '<':   pass = lhs.lt(rhs); break;
    default:
      return { ...base, status: 'SKIPPED_UNSUPPORTED_OP', lhs: lhs.toFixed(), rhs: rhs.toFixed(), gap: null, skipReason: op };
  }

  if (pass) {
    return { ...base, status: 'PASS', lhs: lhs.toFixed(), rhs: rhs.toFixed(), gap: null, skipReason: null };
  }
  return {
    ...base,
    status: 'FAIL',
