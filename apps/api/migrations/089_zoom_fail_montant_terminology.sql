-- Migration 089_zoom_fail_montant_terminology.sql
-- Object: replace the math-engineering terminology "membre gauche /
--         membre droit / LHS / RHS" in the active aggregate_zoom_fail
--         prompt with the banker-native vocabulary "montant calculé /
--         montant attendu". The user feedback: "le membre gauche ou
--         le membre droit sont définie regalica doit reconnaitre
--         montant calculé et montant reçu non pas le membre gauche ou
--         le membre droit".
--
-- Author: ALGORIA Factory
-- Date: 2026-05-06
-- Depends on: 086_zoom_fail_assertive_voice.sql.
...
BEGIN
  ...
  OR current_setting('app.seed_author_user_id', true) = '' THEN
    RAISE NOTICE 'migration 089: GUCs not set - skipping update';
    RETURN;
  END IF;
...
backtick triple, aucune signature de fin.$TMPL$,
  ...
  RAISE NOTICE 'migration 089: aggregate_zoom_fail terminology aligned to montant calculé/attendu (% rows)', v_updated;
END $$;
