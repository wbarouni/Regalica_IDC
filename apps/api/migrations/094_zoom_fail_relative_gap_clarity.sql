-- Migration 094_zoom_fail_relative_gap_clarity.sql
-- Object: refine the BLOC 2 table rendering rules in
--         `regalica/aggregate_zoom_fail` so the « Écart relatif » row
--         is OMITTED when `rhs = 0` (mathematically undefined ratio).
--         The previous template rendered "non transmis %" for these
--         cases — technically accurate but unparlable for a banker.
--         The new wording: when rhs = 0 the row simply does not
--         appear, and BLOC 1's "écart total" qualifier already
--         conveys the meaning (the absolute gap equals the computed
--         amount in full).
--
-- Author: ALGORIA Factory
-- Date: 2026-05-07
-- Depends on: 093_zoom_fail_drop_rdg_self_citation.sql.
...
BEGIN
  ...
  OR current_setting('app.seed_author_user_id', true) = '' THEN
    RAISE NOTICE 'migration 094: GUCs not set - skipping update';
    RETURN;
  END IF;
...
backtick triple, aucune signature de fin.$TMPL$,
  ...
  RAISE NOTICE 'migration 094: aggregate_zoom_fail omits Ecart relatif row when rhs=0 (% rows)', v_updated;
END $$;
