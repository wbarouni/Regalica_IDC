...
the LLM fields (LLM_PROVIDER, GEMINI_MODEL, OLLAMA_URL, OLLAMA_MODEL,
EMBEDDING_PROVIDER, EMBEDDING_MODEL) are required with no defaults (per
the zero-hardcoding doctrine, docs/03 Niveau 3). Without values in the
process environment, any `from app.main import app` in a test would raise
a pydantic ValidationError during collection.
